    ...
    const handleScroll = () => {
      if (containerRef.current) {
        ...
        setShowBottomOverlay(
          (overflow === 'y' || overflow === 'both') && scrollTop + clientHeight < scrollHeight,
        )
        setShowLeftOverlay((overflow === 'x' || overflow === 'both') && scrollLeft > 0)
        setShowRightOverlay(
          (overflow === 'x' || overflow === 'both') && scrollLeft + clientWidth < scrollWidth,
        )
      }
    }
  ...
  return (
    <div className="relative flex flex-col gap-2 overflow-hidden" style={{ width, height }}>
      ...
      {withButtons && overflow === 'x' && (
        <div className="scroller-nav z-10 m-[1px] flex gap-2">
          <Button
      ...
      <div
        className={clsx(
          'pointer-events-none absolute right-0 left-0 h-10 w-full bg-gradient-to-b from-[#0a0a0a] to-transparent transition-opacity duration-500 ease-[cubic-bezier(0.22,1,0.36,1)]',
          showTopOverlay ? (withButtons ? 'top-10' : 'top-0') : '-top-10',
        )}
      />
      <div
        className={clsx(
          'pointer-events-none absolute right-0 left-0 h-10 w-full bg-gradient-to-t from-[#0a0a0a] to-transparent transition-opacity duration-500 ease-[cubic-bezier(0.22,1,0.36,1)]',
          showBottomOverlay ? 'bottom-0' : '-bottom-10',
        )}
      />
      <div
        className={clsx(
          'pointer-events-none absolute top-0 bottom-0 h-full w-10 bg-gradient-to-r from-[#0a0a0a] to-transparent transition-opacity duration-500 ease-[cubic-bezier(0.22,1,0.36,1)]',
          showLeftOverlay ? 'left-0' : '-left-10',
        )}
      />
      <div
        className={clsx(
          'pointer-events-none absolute top-0 bottom-0 h-full w-10 bg-gradient-to-l from-[#0a0a0a] to-transparent transition-opacity duration-500 ease-[cubic-bezier(0.22,1,0.36,1)]',
